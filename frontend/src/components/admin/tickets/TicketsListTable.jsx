...
function TicketsListTable({ data }) {
    ...
    return (
        <div className="flex overflow-hidden flex-wrap items-start mt-6 mr-6 ml-6 rounded-md shadow-sm font-['Amble'] bg-neutral-50 max-md:mr-2.5">
            <TableColumn title="Ticket ID" items={data.map((item) => item.ticketId)} emptyCount={emptyRows} />
            <TableColumn title="User Name" items={data.map((item) => item.name)} emptyCount={emptyRows} />
            <TableColumn title="Issue Type" items={data.map((item) => item.issueType)} emptyCount={emptyRows} />
            <TableColumn title="Date Submitted" items={data.map((item) => item.dateSubmitted)} width="w-[185px]" emptyCount={emptyRows} />
            <TableColumn
                title="Status"
                items={data.map((item) => ({
                    text: item.status,
            ...
            />
            <ActionsColumn data={data} rowCount={data.length} emptyCount={emptyRows} />
        </div>
    );
}
                    ...
                    >
                        <div className="flex flex-1 shrink gap-2.5 items-center self-stretch px-6 py-[19px] my-auto w-full rounded-none basis-0 max-md:px-5">
                            <div className="flex items-start self-stretch my-auto w-9">
                                {isResolved ? (
                                    <div className="bg-blue-200 px-5 py-2 rounded text-sm">
                                        Closed
                                    </div>
                                ) : (
